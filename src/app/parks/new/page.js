...
export default function Page() {
    ...
    const handleSubmit = (e) => {
        e.preventDefault()
        if(!data.name || !data.urlStream || !data.latitude || !data.longitude){
            setWarning(true)
            return
        }
        ...
        <div className="w-full mt-5 px-20">
            ...
            <div className="flex w-full">
                ...
                <div className="w-[40%]">
                    ...
                    <div className="relative border-3 p-1 border-[#ee9559] rounded-md overflow-hidden" onMouseEnter={() => setHoverImg(true)} onMouseLeave={() => setHoverImg(false)}>
                        <Image src="https://w.wallhaven.cc/full/yx/wallhaven-yx1j27.jpg" alt="park" className="aspect-video w-full z-0" />
                        <motion.div 
                            className="absolute top-[50%] left-[50%] translate-x-[-50%] translate-y-[-50%] 
                            bg-white z-20 aspect-video w-full flex items-center justify-center bg-opacity-60
                            flex-col gap-3"
                            animate={hoverImg ? {opacity: 1} : {opacity: 0}}
                        >   
